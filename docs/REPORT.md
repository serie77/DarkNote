...
DarkNote is a pre-existing application, authored by the student before this assessment, that lets a user send an encrypted note to any Solana wallet address. Encryption is performed in the browser with NaCl's `box` construction (X25519 key agreement with the XSalsa20–Poly1305 authenticated cipher); the recipient's encryption keypair is derived deterministically from a wallet signature so that only the wallet owner can decrypt; and the server persists only ciphertext, a nonce and an ephemeral public key. The server therefore *cannot* read messages — a genuine end-to-end (zero-access) posture rather than a policy promise. The exact construction as implemented is set out in Section 2.5.
